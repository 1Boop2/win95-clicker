...
function check_achievements(int $uid): array {
  ...
  foreach($defs as $d){ $c=$d['code']; if(!empty($unlocked[$c])) continue; if($d['type']==='stat'){ $field=$d['field']; $gte=(float)$d['gte']; $v=(float)($vals[$field]??0); if($v>=$gte){ $ins->execute([$uid,$c]); $new[]=$c; } } }
  return $new;
}
